import React from "react";
import { Grid, Typography, Card } from "@mui/material";
import { useRouter } from "next/router";

// styles
import sxStyles from "../../styles/style";
import Link from "next/link";

function VsComponentDesk(props) {
  const router = useRouter();
  return (
    <Grid lg={4} md={4} sm={6} xs={12} className={"textCenter p-4"}>
      <Link href={"/" + props.url}>
        <a>
          <Card className={"vsCard"}>
            <div className={"vsCardInn"}>
              <Grid container>
                <Grid xs={4} sx={{ alignSelf: "center" }}>
                  <Card className={"imgCard"}>
                    <img
                      alt={props.image1?.alt}
                      src={
                        props.image1?.fileName
                          ? process.env.NEXT_PUBLIC_IMAGE_SERVER +
                            "/files/images/big/" +
                            props.image1?.fileName
                          : process.env.NEXT_PUBLIC_IMAGE_SERVER +
                            "/files/images/placeholder/brand-logo.webp"
                      }
                    />
                  </Card>
                </Grid>
                <Grid xs={4} sx={{ alignSelf: "center" }}>
                  <Typography
                    variant="body1"
                    sx={{
                      fontSize: "20px",
                    }}
                  >
                    Vs
                  </Typography>
                </Grid>
                <Grid xs={4} sx={{ alignSelf: "center" }}>
                  <Card className={"imgCard"}>
                    <img
                      alt={props.image2?.alt}
                      src={
                        props.image2?.fileName
                          ? process.env.NEXT_PUBLIC_IMAGE_SERVER +
                            "/files/images/big/" +
                            props.image2?.fileName
                          : process.env.NEXT_PUBLIC_IMAGE_SERVER +
                            "/files/images/placeholder/brand-logo.webp"
                      }
                    />
                  </Card>
                </Grid>
              </Grid>
            </div>
          </Card>
          <Grid container>
            <Grid xs={4} alignSelf={"center"} mr={"auto"} textAlign={"center"}>
              <Typography
                variant="body1"
                className={"brName"}
                fontWeight={"bolder"}
                fontSize={12}
              >
                {props.title1}
              </Typography>
            </Grid>
            <Grid xs={4} alignSelf={"center"} ml={"auto"} textAlign={"rcenter"}>
              <Typography
                variant="body1"
                className={"brName"}
                fontWeight={"bolder"}
                fontSize={12}
              >
                {props.title2}
              </Typography>
            </Grid>
          </Grid>
        </a>
      </Link>
    </Grid>
  );
}

export default VsComponentDesk;
